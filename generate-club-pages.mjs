      ...
      <div class="ctf-footer-grid" style="padding-bottom:48px;border-bottom:1px solid rgba(255,255,255,.06);">
        ...
        <div>
          ...
          <div style="display:flex;gap:8px;">
            ...
            <a href="https://www.instagram.com/ctsoccerhub/" target="_blank" rel="noopener" aria-label="Instagram" style="width:34px;height:34px;border-radius:8px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;transition:background .2s;" onmouseover="this.style.background='rgba(255,255,255,.13)'" onmouseout="this.style.background='rgba(255,255,255,.07)'">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.55)" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" stroke-linecap="round"/></svg>
            </a>
        ...
        <div>
          ...
          <ul style="list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:10px;">
            ...
            <li><a href="../season-review.html" style="font-size:.875rem;color:rgba(255,255,255,.38);text-decoration:none;transition:color .2s;" onmouseover="this.style.color='rgba(255,255,255,.85)'" onmouseout="this.style.color='rgba(255,255,255,.38)'">Season in Review</a></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul style="list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:10px;">
            ...
            <li><a href="../data-sources.html" style="font-size:.875rem;color:rgba(255,255,255,.38);text-decoration:none;transition:color .2s;" onmouseover="this.style.color='rgba(255,255,255,.85)'" onmouseout="this.style.color='rgba(255,255,255,.38)'">Data Sources</a></li>
...
// ---- Sitemap: replace a marked "CLUB PAGES" block, leaving everything else
// untouched. On the first run this also strips whatever unmarked clubs/*.html
// <url> entries were previously added by hand (there's no other way to tell
// "an old manual entry" from "a page that should stay" once this block owns
// the section) — after that, every run just rebuilds the marked block, so a
// club added to clubs-data.js can't silently go missing from the sitemap the
// way it could before. Pro/semi-pro clubs (no `type: 'youth'`) get the same
// higher 0.7 priority they already had; youth/EDP clubs stay at 0.5. ----
function updateSitemap(clubs) {
  const sitemapPath = path.join(__dirname, 'sitemap.xml');
  let xml = fs.readFileSync(sitemapPath, 'utf8');
  const startMarker = '  <!-- CLUB PAGES (auto-generated by generate-club-pages.mjs — do not hand-edit this block) -->';
  const endMarker = '  <!-- END CLUB PAGES -->';

  // Remove the marked block if this has run before.
  const oldStart = xml.indexOf(startMarker);
  const oldEnd = xml.indexOf(endMarker);
  if (oldStart !== -1 && oldEnd !== -1) {
    xml = xml.slice(0, oldStart) + xml.slice(oldEnd + endMarker.length);
  }
  // One-time migration: strip any leftover unmarked clubs/*.html entries
  // from before this generator managed the section (no-op once migrated).
  xml = xml.replace(/[ \t]*<url>\s*<loc>https:\/\/ctsoccerhub\.com\/clubs\/[^<]*<\/loc>[\s\S]*?<\/url>\s*\n?/g, '');

  const block = clubs.map((c) => {
    const priority = c.type === 'youth' ? '0.5' : '0.7';
    return `\n  <url>\n    <loc>https://ctsoccerhub.com/clubs/${c.id}.html</loc>\n    <changefreq>weekly</changefreq>\n    <priority>${priority}</priority>\n  </url>`;
  }).join('') + '\n';
...
updateSitemap(ALL_CLUBS);
console.log('Updated sitemap.xml (CLUB PAGES block)');
